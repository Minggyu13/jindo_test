...
def main():
    ...
    with st.sidebar:
        ...
        st.session_state.add_task_input = st.text_input("할 일 추가", key="add_task_input")
        if st.button("추가"):
            add_task(st.session_state.add_task_input)

        # 할 일 삭제 입력 필드의 max_value 수정
        delete_task_input = st.number_input("할 일 삭제 (번호 입력)", 
                                            min_value=1, 
                                            max_value=max(len(st.session_state['to_do_list']), 1), 
                                            step=1, 
                                            key="delete_task")
        if st.button("삭제"):
